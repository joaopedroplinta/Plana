...
export default function TenantsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Salões</h1>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-gray-50">
              <th className="px-5 py-3 text-left font-medium text-gray-600">Salão</th>
              ...
              <th className="px-5 py-3 text-right font-medium text-gray-600">Usuários</th>
              <th className="px-5 py-3 text-right font-medium text-gray-600">Criado em</th>
            </tr>
          </thead>
          <tbody>
            {isLoading
              ? Array.from({ length: 5 }).map((_, i) => (
                  <tr key={i} className="border-b animate-pulse">
                    ...
                    <td className="px-5 py-4 text-center">
                      <div className="mx-auto h-5 w-10 rounded-full bg-gray-200" />
                    </td>
                    <td className="px-5 py-4 text-right">
                      <div className="ml-auto h-4 w-6 rounded bg-gray-200" />
                    </td>
                    <td className="px-5 py-4 text-right">
                  ...
                  <tr key={tenant.id} className="border-b last:border-0 hover:bg-gray-50">
                    ...
                    <td className="px-5 py-4">
                      <select
                        ...
                        className={`rounded-full px-2.5 py-0.5 text-xs font-semibold border-0 cursor-pointer focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-50 ${PLAN_COLORS[tenant.plan] ?? 'bg-gray-100 text-gray-700'}`}
                      >
                        {PLAN_OPTIONS.map((p) => (
                    ...
                    <td className="px-5 py-4 text-center">
                      <button
                        ...
                        disabled={updating === tenant.id}
                        className={`inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-50 ${tenant.active ? 'bg-green-500' : 'bg-gray-300'}`}
                    ...
                    <td className="px-5 py-4 text-right text-gray-700">{tenant.user_count}</td>
